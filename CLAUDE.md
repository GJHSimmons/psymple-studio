# Psymple Studio — Claude Working Instructions
...
Psymple Studio is a lightweight but stable graphical interface over
[`psymple`](https://github.com/casasglobal-org/psymple), a compositional ODE modelling
package. Users assemble models out of *ported objects* — leaf nodes carrying assignments
(`VariablePortedObject` for ODEs, `FunctionalPortedObject` for functions) and
`CompositePortedObject` nodes that nest children and wire their ports together — then
compile the tree into a flat simulable system and run a numerical integration. Studio is
the visual front for that workflow: a nested-canvas builder, a compilation view, and a
simulation view.
...
plus the psymple-adjacent code described under *Code style boundaries*, which is written
here but destined for a future psymple release.
...
- **psymple-adjacent** (`psymple_ext/`) — capabilities that are really psymple features:
  ...
  *psymple's own style* — a research-engineering project: pragmatic, not over-complete, not
  exhaustively tested, matching psymple's module layout, naming, and docstring conventions
...
Mark **hub** on every file that everything registers into — the conflict-avoidance rules in
Part 2 read this marking.

| Path | What it is | Hub |
|---|---|---|
| `CLAUDE.md` | This file: Part 1 + shared protocol | **hub** |
| `docs/ROADMAP.md` | Wave completion criteria; the roadmap named in bindings | **hub** |
| `frontend/` | React + Vite + TS app — the Studio UI | |
...
The cut keeps the two big parallel efforts (Builder, Screens) off each other's hub files —
both only *consume* the design tokens and the engine interface.
